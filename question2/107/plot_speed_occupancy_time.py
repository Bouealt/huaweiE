...
# 视频的开始时间
start_time = datetime.strptime('11:41:03', '%H:%M:%S')  # 替换为您的视频开始时间
...
ax2.scatter(df['Time'], df['Emergency Lane Flow Rate'], s=df['Emergency Lane Flow Rate']*500,
            color='#ea152d', alpha=0.8, label='Emergency Lane Occupancy Rate (%)')
ax2.tick_params(axis='y', labelcolor='#24b5db')
ax2.set_ylim(0, 0.5)  # 设置应急车道占有率 Y 轴的上下限为 0 到 10

# 设置图例和标题
plt.title('Speed and Emergency Lane Occupancy Rate Over Time（点位1）', fontsize=18)
...
# 显示图表
plt.tight_layout()
plt.savefig('点位1时间、应急车道、速度关系图.png')
plt.show()
